...
| WC-024 | Club Connections (and Discoveries) showed only ~11 players on live data | clubs · discoveries · club-affiliation precompute | The `clubs.json` map was keyed by **API-Football** player ids, but live-2026 migrated to **SportMonks** ids — two namespaces that never join, so only ~11 coincidental numeric collisions matched (and could point at the wrong club). | Rebuilt the join as a cross-provider crosswalk on **surname + birthdate**: the precompute now pulls full bios (name/DOB/nationality) via API-Football `/players?team=&season=` and emits a `byKey` (`"surname\|dob"`) map (13,670 keys / 32 leagues / 1,074 calls); SportMonks players now carry `birthDate`; a shared `clubMatchKey()` (script + app) builds the key; `clubs.ts` + `discoveries.ts` join on it. Verified live: **528 players linked (was 11)** with correct mappings (Liverpool → Gakpo/Isak/van Dijk/Mac Allister). · fetch-clubs.mjs · clubAffiliations.ts · clubs.ts · discoveries.ts · sportmonks.ts · types.ts |

...
| ✅ | **Rotating briefing deck** (BRIEF-2) | The home hero is now a deck that cycles a different story every 15s — title race, live recaps (with scorers, never declaring a live winner), recent-result recaps with xG context, golden boot, momentum, the meanest defense, biggest rout, upset, breakout, marquee fixtures. Progress bar + dot nav + prev/next + hover-pause + play/pause; auto-rotation off under reduced-motion. (`generateBriefingDeck` in `src/ai/narratives.ts`, `src/components/home/BriefingDeck.tsx`) |

...
| ✅ | **Deep-round previews + conditional title math** (`/scenarios`, "The Business End") | Comprehensive QF/SF/Final tie previews with the **conditional title picture**: for each side, title odds now vs *if they win tonight*, via the identity `P(title \| win this tie) = P(title) / P(reach next round)` (both from the same Monte Carlo, so ≤ 1 and self-consistent), plus the per-match advance odds, expected goals, likely score, each team's knockout path and top scorer, and a "tie of the round." No re-simulation — a sound probability identity over numbers the engine already produced. A lightweight, shippable form of the tabled QR-7 scenario solver. (`server/deepRounds.ts`, `app/scenarios/page.tsx`) |
